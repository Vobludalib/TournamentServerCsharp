...
public class ServerHandler
{
    ...
    public async Task<IResult> GetEntrantByIdAsync(int id)
    {
        ...
        var json = JsonSerializer.Serialize(
            entrant,
            new JsonSerializerOptions { WriteIndented = true, Converters = { entrantConverter } }
        );
        return Results.Content(json, "application/json");
    }
                ...
                using (await setToTieTo.LockHandler.LockSetWriteAsync())
                {
                    ...
                    else if (amountOfMatchingSets == 0)
                    {
                        ...
                        if (setToTieTo.Games.Count == 0 && game.GameNumber != 1)
                            return Results.BadRequest("First game must have gameNumber set to 1.");
                        else if (
    ...
    public async Task<IResult> HandleSetMoveWinnersAndLoserAsync(int setId)
    {
        ...
        using (var entrantsLock = await tournament.LockHandler.LockEntrantsReadAsync())
        {
            ...
            try
            {
                ...
                if (success)
                    return Results.Ok("Set winner and loser have been moved to their next sets.");
    ...
    public async Task<IResult> HandleGameUpdateWinnerAsync(int setId, int gameNumber, int entrantId)
    {
        {
            if (tournament is null)
            {
